...
export const make = Effect.gen(function* () {
  ...
  const browse: WorkspaceEntries["Service"]["browse"] = Effect.fn("WorkspaceEntries.browse")(
    function* (input) {
      ...
      // Existing filesystem-directory browsing intentionally reveals hidden
      // folders after a path separator. Composer file previews should not dump
      // .git/node metadata merely because they browse the workspace root.
      const showHidden =
        input.includeFiles === true
          ? prefix.length > 0 && prefix.startsWith(".")
          : endsWithSeparator || prefix.startsWith(".");
      ...
      for (const dirent of dirents) {
        ...
        const isIncludedFile = input.includeFiles === true && dirent.isFile();
        if (
          (isDirectory || isIncludedFile) &&
      ...
      return {
        parentPath,
        entries: input.limit ? sortedEntries.slice(0, input.limit) : sortedEntries,
      };
    },
  );
