...
import { mergeStableModelContext } from "../modelContextStability.ts";
import {
...
const mergeProviderModels = (
  ...
  const mergedModels = nextModels.map((model) => {
    ...
    const stableModel = mergeStableModelContext(previousModel, model);
    if (hasModelCapabilities(stableModel) || !hasModelCapabilities(previousModel)) {
      return stableModel;
    }
    return {
      ...stableModel,
    ...
    const syncLiveSources = syncSemaphore.withPermits(1)(
      Effect.gen(function* () {
        ...
        for (const [, instance] of newlyAdded) {
          ...
          const sourceChanges = source.subscribeChanges
