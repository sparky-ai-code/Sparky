...
function shouldTrackRpcAck(tag: string): boolean {
  // Command dispatch is an acknowledgement of control-plane intent. The
  // actual model turn is delivered on the thread stream, so showing a
  // generic "slow RPC" warning for a queued dispatch falsely reports model
  // generation latency.
  const isOrchestrationDispatch = tag.startsWith(`${ORCHESTRATION_WS_METHODS.dispatchCommand} `);
  return !tag.includes("subscribe") && !untrackedRpcAckTags.has(tag) && !isOrchestrationDispatch;
}
