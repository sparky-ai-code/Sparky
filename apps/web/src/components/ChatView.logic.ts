...
/**
 * Read attachments serially. Multiple callers may still enqueue work at once,
 * but only one FileReader is active at a time so selecting several files does
 * not spike memory/CPU and stall lower-powered devices.
 */
export function readFileAsDataUrl(file: File): Promise<string> {
  ...
  const result = fileReadQueue.then(read, read);
  fileReadQueue = result.then(
    () => undefined,
    () => undefined,
  );
  return result;
