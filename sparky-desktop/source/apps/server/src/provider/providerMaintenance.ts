...
export const resolveProviderMaintenanceCapabilitiesEffect = Effect.fn(
  ...
  // Maintenance classification is optional metadata. A provider instance
  // must not remain unbuilt because PATH/filesystem discovery stalls (this is
  // especially important while replacing an instance after settings change).
  // Fall back to the resolver's conservative package/manual classification
  // when lookup fails or exceeds the short discovery budget.
  const resolvedCommandPathOption = yield* resolveCommandPath(binaryPath, { env }).pipe(
    Effect.timeoutOption(COMMAND_LOOKUP_TIMEOUT_MS),
    Effect.orElseSucceed(() => Option.none()),
  );
  const resolvedCommandPath =
    Option.getOrNull(resolvedCommandPathOption) ?? (hasPathSeparator(binaryPath) ? binaryPath : null);
  if (!resolvedCommandPath) {
    return resolver.resolve(options);
  }
