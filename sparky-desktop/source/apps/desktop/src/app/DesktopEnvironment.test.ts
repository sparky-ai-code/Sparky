...
describe("DesktopEnvironment", () => {
  ...
  it.effect(
    "prefers the unpacked Windows taskbar icon without changing development resources",
    () =>
      Effect.gen(function* () {
        const packaged = yield* makeEnvironment({ isPackaged: true, platform: "win32" });
        const development = yield* makeEnvironment({ platform: "win32" });

        assert.equal(
          packaged.resolveResourcePathCandidates("icon.ico")[0],
          packaged.path.join(packaged.resourcesPath, "icon.ico"),
        );
        assert.equal(
          development.resolveResourcePathCandidates("icon.ico")[0],
          development.path.join(defaultInput.dirname, "../resources", "icon.ico"),
        );
        assert.equal(
          packaged.resolveResourcePathCandidates("icon.png")[0],
          packaged.path.join(defaultInput.dirname, "../resources", "icon.png"),
        );
      }),
  );
