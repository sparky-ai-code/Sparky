...
describe("deriveWorkLogEntries", () => {
  it("hides internal plugin authorization requests and keeps plugin identity on real calls", () => {
    const entries = deriveWorkLogEntries([
      makeActivity({
        id: "plugin-auth-request",
        summary: "Sparky Request Plugin Authorization",
        payload: {
          itemType: "mcp_tool_call",
          data: {
            toolName: "sparky_request_plugin_authorization",
            item: { name: "sparky_request_plugin_authorization", arguments: { pluginId: "gmail" } },
          },
        },
      }),
      makeActivity({
        id: "plugin-call",
        summary: "Sparky plugin call",
        kind: "tool.completed",
        payload: {
          itemType: "mcp_tool_call",
          data: {
            ...
            item: {
              ...
              arguments: { pluginId: "gmail", action: "search_messages", input: {} },
            },
          },
        },
      }),
    ...
    expect(entries.map((entry) => entry.id)).toEqual(["plugin-call"]);
    expect(entries[0]?.toolData).toMatchObject({ pluginId: "gmail" });
  });
  ...
  it("retains structured web-search results on the completed timeline entry", () => {
    const [entry] = deriveWorkLogEntries([
      makeActivity({
