...
const PickFileOrFolderOptionsSchema = Schema.Struct({
  initialPath: Schema.optionalKey(Schema.NullOr(Schema.String)),
  targetEnvironmentId: Schema.optionalKey(Schema.String),
  kind: Schema.Literals(["file", "folder", "any"]),
});
...
function resolvePickerTarget(options: { targetEnvironmentId?: string } | undefined) {
  ...
  return { targetId, wslDistroFromTarget, useWsl };
}
...
export const pickFolder = DesktopIpc.makeIpcMethod({
  ...
  handler: Effect.fn("desktop.ipc.window.pickFolder")(function* (options) {
    ...
    const { wslDistroFromTarget, useWsl } = resolvePickerTarget(options);
    ...
    if (!useWsl) return selectedPath.value;
...
export const pickFileOrFolder = DesktopIpc.makeIpcMethod({
  channel: IpcChannels.PICK_FILE_OR_FOLDER_CHANNEL,
  payload: PickFileOrFolderOptionsSchema,
  result: Schema.NullOr(Schema.String),
  handler: Effect.fn("desktop.ipc.window.pickFileOrFolder")(function* (options) {
    const dialog = yield* ElectronDialog.ElectronDialog;
    const electronWindow = yield* ElectronWindow.ElectronWindow;
    const environment = yield* DesktopEnvironment.DesktopEnvironment;
    const appSettings = yield* DesktopAppSettings.DesktopAppSettings;
    ...
    const defaultPath = useWsl
      ...
      : environment.resolvePickFolderDefaultPath(options);
    const selectedPath = yield* dialog.pickFileOrFolder({
      owner: yield* electronWindow.focusedMainOrFirst,
      defaultPath,
      kind: options.kind,
    });
