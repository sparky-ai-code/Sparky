...
function makeFakeBrowserWindow() {
  ...
  const webContentsListeners = new Map<string, (...args: readonly unknown[]) => unknown>();
  const webContents = {
    copyImageAt: vi.fn(),
    ...
    send: vi.fn(),
    setWindowOpenHandler: vi.fn((listener: (...args: readonly unknown[]) => unknown) => {
      webContentsListeners.set("set-window-open-handler", listener);
    }),
  };
...
describe("DesktopWindow", () => {
  ...
  it.effect("reapplies the packaged Windows icon after native window creation", () =>
    Effect.gen(function* () {
      const fakeWindow = makeFakeBrowserWindow();
      const createCount = yield* Ref.make(0);
      const mainWindow = yield* Ref.make<Option.Option<Electron.BrowserWindow>>(Option.none());
      const iconPath = "C:/Program Files/Sparky/resources/icon.ico";
      const createdWindowOptions: Electron.BrowserWindowConstructorOptions[] = [];
      const layer = makeTestLayer({
        window: fakeWindow.window,
        createCount,
      ...
      yield* Effect.gen(function* () {
        ...
        assert.deepEqual(fakeWindow.setIcon.mock.calls, [[iconPath]]);
      }).pipe(Effect.provide(layer));
    }),
  );
  ...
  it.effect("recovers a maximized window that Electron places off-screen", () =>
    Effect.gen(function* () {
      const fakeWindow = makeFakeBrowserWindow();
      fakeWindow.getBounds.mockReturnValue({ x: -25_600, y: -25_600, width: 1100, height: 780 });
      const createCount = yield* Ref.make(0);
      const mainWindow = yield* Ref.make<Option.Option<Electron.BrowserWindow>>(Option.none());
      const layer = makeTestLayer({
        window: fakeWindow.window,
  ...
  it.effect("hands Clerk blank OAuth popups to the system browser after navigation", () =>
    Effect.gen(function* () {
      const fakeWindow = makeFakeBrowserWindow();
      ...
      yield* Effect.gen(function* () {
        ...
        const windowOpenHandler = fakeWindow.webContentsListeners.get("set-window-open-handler") as
          | ((details: { url: string }) => Electron.WindowOpenHandlerResponse)
          | undefined;
        const didCreateWindow = fakeWindow.webContentsListeners.get("did-create-window");
        if (!windowOpenHandler || !didCreateWindow) {
          return yield* Effect.die("OAuth popup listeners were not registered");
        }
        ...
        didCreateWindow(popup.window, { url: "about:blank" });

        const popupWillNavigate = popup.webContentsListeners.get("will-navigate");
        if (!popupWillNavigate) {
          return yield* Effect.die("OAuth popup navigation listener was not registered");
        }
        let prevented = false;
        popupWillNavigate(
          {
        ...
        assert.isTrue(prevented);
        assert.deepEqual(openedExternalUrls, ["https://accounts.google.com/o/oauth2/auth"]);
        assert.equal(popup.close.mock.calls.length, 1);
      }).pipe(Effect.provide(layer));
    }),
  );
