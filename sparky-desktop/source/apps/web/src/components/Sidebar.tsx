...
import { getModelProviderPresentation } from "./chat/providerIconUtils";
...
const SIDEBAR_ICON_ACTION_BUTTON_CLASS =
  "inline-flex h-6 min-w-6 cursor-pointer items-center justify-center rounded-md px-[calc(--spacing(1)-1px)] text-muted-foreground/60 outline-none hover:text-foreground focus:outline-none focus-visible:outline-hidden focus-visible:ring-0 focus-visible:ring-offset-0";
const SIDEBAR_CHATS_KEY = "__sparky_chats__";

function sortThreadsWithPinned(
  threads: readonly SidebarThreadSummary[],
  sortOrder: SidebarThreadSortOrder,
  pinnedThreadKeys: readonly string[],
): SidebarThreadSummary[] {
  const sorted = sortThreads(threads, sortOrder);
  if (pinnedThreadKeys.length === 0) return sorted;

  const pinnedOrder = new Map(pinnedThreadKeys.map((key, index) => [key, index] as const));
  return [...sorted].sort((left, right) => {
    const leftRank = pinnedOrder.get(scopedThreadKey(scopeThreadRef(left.environmentId, left.id)));
    const rightRank = pinnedOrder.get(
...
function threadPathSummary(path: string | null): { folder: string; path: string } {
  const normalized = path?.trim().replaceAll("\\", "/").replace(/\/+$/, "") ?? "";
  const parts = normalized.split("/").filter(Boolean);
  return {
    folder: parts.at(-1) ?? "Chat",
    path: normalized || "No project path",
  };
}
...
export const SidebarThreadRow = memo(function SidebarThreadRow(props: SidebarThreadRowProps) {
  ...
  const threadPath = thread.worktreePath ?? threadProjectCwd ?? props.projectCwd;
  const pathSummary = threadPathSummary(threadPath);
  ...
  useRelativeTimeTick(
    threadStatus?.label === "Working" || threadStatus?.label === "Connecting" ? 1_000 : 60_000,
  );
  const runDurationLabel = formatThreadRunDuration(thread);
  const modelProvider = getModelProviderPresentation({ slug: thread.modelSelection.model });
  const ModelProviderIcon = modelProvider?.Icon ?? null;
  const pr = resolveThreadPr(thread.branch, gitStatus.data);
  const prStatus = prStatusIndicator(pr, gitStatus.data?.sourceControlProvider);
  const terminalStatus = terminalStatusFromRunningIds(runningTerminalIds);
  const isConfirmingArchive = confirmingArchiveThreadKey === threadKey && !isThreadRunning;
  const isDimmedInFocusMode = focusMode && threadStatus?.label !== "Completed";
  ...
  const handleMenuTriggerClick = useCallback((event: React.MouseEvent<HTMLButtonElement>) => {
    event.stopPropagation();
  }, []);
    ...
    >
      <SidebarMenuSubButton
        ...
        className={`${resolveThreadRowClassName({
          isActive,
          isSelected,
        })} relative isolate !h-auto min-h-20 items-start gap-2 py-2.5 ${
          isDimmedInFocusMode ? "opacity-55 grayscale-[0.65]" : ""
        }`}
      ...
      >
        <div className="flex min-w-0 flex-1 flex-col gap-1 text-left">
            ...
            {renamingThreadKey === threadKey ? (
              ...
              <Tooltip>
                <TooltipTrigger
                  render={
                    <span
                      className="min-w-0 flex-1 line-clamp-2 overflow-hidden text-xs font-medium leading-snug text-foreground/90"
          ...
          <div
            className="flex min-w-0 items-center gap-1 truncate text-[10px] text-muted-foreground/65"
            title={`${pathSummary.folder} · ${pathSummary.path}`}
          >
            {isRemoteThread && !isDesktopLocalThread ? (
              <CloudIcon className="size-3 shrink-0 text-muted-foreground/45" aria-hidden="true" />
            ) : null}
            {isPinned ? (
              <span
                className="inline-flex size-3 shrink-0 items-center justify-center text-primary/80"
                role="img"
                aria-label="Pinned thread"
                title="Pinned thread"
              >
                <PinIcon className="size-3" fill="currentColor" aria-hidden="true" />
              </span>
            ) : null}
            ...
            <span className="min-w-0 truncate">{pathSummary.path}</span>
          </div>
          <div className="flex min-w-0 items-center gap-2 truncate text-[10px]">
            {threadStatus ? (
              <span
                className={`inline-flex min-w-0 items-center gap-1 font-medium ${threadStatus.colorClass}`}
              ...
              >
                {threadStatus.label === "Working" || threadStatus.label === "Connecting" ? (
                  ...
                  <span
                    className={`size-1.5 shrink-0 rounded-full ${threadStatus.dotClass}`}
                    aria-hidden="true"
                  />
                )}
                <span className="truncate">
                  {threadStatus.label === "Error" ? "Failed" : threadStatus.label}
                </span>
              </span>
            ) : null}
            {runDurationLabel ? (
              <span
                className={`shrink-0 tabular-nums ${
                  isThreadRunning ? "text-sky-500 dark:text-sky-300" : "text-muted-foreground/70"
                }`}
                title={`Agent run duration: ${runDurationLabel}`}
              >
                {runDurationLabel}
              </span>
            ) : null}
            <span
              className="shrink-0 tabular-nums text-muted-foreground/65"
              title={`Last interacted ${formatRelativeTimeLabel(
        ...
        <div className="ml-2 flex shrink-0 items-start gap-1">
          ...
          <div className="flex shrink-0 flex-col items-center gap-0.5">
            {ModelProviderIcon ? (
              <span
                className="inline-flex size-6 items-center justify-center text-muted-foreground/75"
                role="img"
            ...
            <Menu>
              <MenuTrigger
                render={
                  <button
                    ...
                    className={`${SIDEBAR_ICON_ACTION_BUTTON_CLASS} size-6 text-muted-foreground/55 hover:bg-accent hover:text-foreground`}
              ...
              <MenuPopup align="end" side="bottom">
                ...
                >
                  <PinIcon className="size-3.5" fill={isPinned ? "currentColor" : "none"} />
            ...
            {isConfirmingArchive ? (
              <button
                ...
                className="inline-flex h-6 min-w-7 cursor-pointer items-center justify-center rounded-md bg-destructive/12 px-1.5 text-[10px] font-medium text-destructive transition-colors hover:bg-destructive/18 focus-visible:outline-hidden focus-visible:ring-1 focus-visible:ring-destructive/40"
                onPointerDown={stopPropagationOnPointerDown}
...
const SidebarProjectItem = memo(function SidebarProjectItem(props: SidebarProjectItemProps) {
  ...
  const handleRemoveProject = useCallback(
    async (member: SidebarProjectGroupMember) => {
      ...
      // The thread count above is a UI snapshot and can be stale by the time
      // the confirmation resolves. A confirmed removal must therefore carry
      // the explicit force flag so the server can atomically remove any
      // threads that appeared after the snapshot was read.
      const result = await removeProject(member, { force: true });
      if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
        const error = squashAtomCommandFailure(result);
...
function SidebarBrand({ onBackdrop }: { onBackdrop: boolean }) {
  return (
    <Link
      ...
      to="/"
    >
      <SparkyLogo aria-hidden="true" className="size-7 shrink-0" />
      <SparkyWordmark />
    </Link>
  );
}
...
const SidebarChatsSection = memo(function SidebarChatsSection(props: SidebarChatsSectionProps) {
  ...
  const visibleThreads = useMemo(
    ...
    [pinnedThreadKeys, threadSortOrder, threads],
  );
          ...
          <TooltipTrigger
            render={
              <button
                ...
                className={`${SIDEBAR_ICON_ACTION_BUTTON_CLASS} text-muted-foreground/60 hover:bg-white/5 hover:text-foreground`}
      ...
      <SidebarGroup className="px-2 pt-2 pb-1">
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton
              size="sm"
              className="gap-2 bg-transparent px-2 py-1.5 font-medium text-muted-foreground/80 outline-none hover:bg-white/5 hover:text-foreground focus:outline-none focus-visible:ring-0 focus-visible:ring-offset-0"
