...
import { formatDuration, isLatestTurnSettled } from "../session-logic";
...
export function formatThreadRunDuration(
  thread: Pick<SidebarThreadSummary, "latestTurn" | "session">,
  nowMs = Date.now(),
): string | null {
  const startedAt = thread.latestTurn?.startedAt;
  ...
  const endedAt = thread.latestTurn.completedAt ?? (isRunning ? null : thread.session?.updatedAt);
  const endedAtMs = endedAt ? Date.parse(endedAt) : nowMs;
  if (!Number.isFinite(endedAtMs) || endedAtMs < startedAtMs) return null;
...
export function hasUnseenCompletion(thread: ThreadStatusInput): boolean {
  const completionAt =
    ...
    (thread.latestTurn?.state === "interrupted" && thread.latestTurn.completedAt !== null)
      ? (thread.latestTurn.completedAt ?? thread.session?.updatedAt ?? null)
      : !thread.latestTurn &&
...
export function hasUnseenError(thread: ThreadStatusInput): boolean {
  const errorAt =
  ...
  if (Number.isNaN(errorAtMs) || !thread.lastVisitedAt) return true;

  const lastVisitedAtMs = Date.parse(thread.lastVisitedAt);
  return Number.isNaN(lastVisitedAtMs) || errorAtMs > lastVisitedAtMs;
}
...
export function resolveThreadRowClassName(input: {
  ...
  if (input.isSelected && input.isActive) {
    return cn(
      baseClassName,
      "!bg-white/[0.1] text-foreground font-medium hover:!bg-white/[0.14] hover:text-foreground dark:!bg-white/[0.1] dark:hover:!bg-white/[0.14]",
    );
  }
  ...
  if (input.isSelected) {
    return cn(
      baseClassName,
      "!bg-white/[0.07] text-foreground hover:!bg-white/[0.11] hover:text-foreground dark:!bg-white/[0.07] dark:hover:!bg-white/[0.11]",
    );
  }
...
export function resolveThreadStatusPill(input: {
  ...
  const hasError =
    (thread.session?.status === "error" ||
      (!hasActiveTurn && thread.latestTurn?.state === "error")) &&
    hasUnseenError(thread);
  ...
  if (thread.hasPendingUserInput) {
    return {
      ...
      colorClass: "text-neutral-700 dark:text-neutral-300/90",
      dotClass: "bg-neutral-500 dark:bg-neutral-300/90",
      pulse: false,
    };
  }
