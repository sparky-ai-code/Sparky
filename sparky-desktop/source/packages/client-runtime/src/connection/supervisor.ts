...
// A WebSocket can remain open after its peer or the local backend has stopped
// responding (for example after sleep, a network transition, or a stalled
// desktop backend). Keep the foreground probe as an immediate check, but also
// probe an otherwise idle session so RPC calls cannot wait forever on a
// half-open transport.
...
> {
  ...
  const monitorConnectedLease = Effect.fnUntraced(function* (
    ...
    const probeConnectedLease = Effect.fnUntraced(function* () {
      yield* lease.session.probe.pipe(
    ...
    for (;;) {
      const next = yield* Effect.raceFirst(
        ...
        Effect.sleep(CONNECTION_HEARTBEAT_INTERVAL).pipe(
          Effect.andThen(probeConnectedLease()),
      ...
      switch (next.signal._tag) {
        case "DisconnectRequested":
        case "RetryRequested":
          return;
        case "NetworkChanged":
          if (next.signal.network === "offline") {
            return;
          }
          break;
        case "Wakeup":
          if (
            next.signal.reason === "credentials-changed" &&
            target._tag === "RelayConnectionTarget"
          ) {
            yield* logManagedRelayAccountChange;
            return;
          }
          if (next.signal.reason === "application-active") {
            const probe = yield* probeConnectedLease().pipe(Effect.forkChild);
            for (;;) {
