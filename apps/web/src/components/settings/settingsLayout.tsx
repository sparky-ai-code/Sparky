...
const SettingsSectionTitleContext = createContext<string | null>(null);
  ...
  return (
    <section {...sectionProps} className={cn("space-y-2.5", className)}>
      <div className="flex items-center justify-between px-1">
        <h2 className="flex items-center gap-2 text-[11px] font-semibold text-foreground/50">
      ...
      <SettingsSectionTitleContext.Provider value={title}>
      ...
      </SettingsSectionTitleContext.Provider>
    </section>
  );
}
...
export function SettingsRow({
  ...
  const sectionTitle = useContext(SettingsSectionTitleContext);
  if (
    sectionTitle === "About" &&
    (title === "Update track" || title === "Diagnostics")
  ) {
    return null;
  }
