...
export const PluginSessionSyncRequest = Schema.Struct({
  sessionToken: Schema.NullOr(Schema.String),
});
export type PluginSessionSyncRequest = typeof PluginSessionSyncRequest.Type;

export const PluginSessionSyncResult = Schema.Struct({
  ready: Schema.Boolean,
});
export type PluginSessionSyncResult = typeof PluginSessionSyncResult.Type;
...
export class EnvironmentPluginsHttpApi extends HttpApiGroup.make("plugins").add(
  HttpApiEndpoint.post("syncSession", "/api/plugins/session", {
    headers: OptionalBearerHeaders,
    payload: PluginSessionSyncRequest,
    success: PluginSessionSyncResult,
    error: EnvironmentScopedOperationErrors,
  }).middleware(EnvironmentAuthenticatedAuth),
) {}
