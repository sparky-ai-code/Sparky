import { EnvironmentId, type RelayClientInstallProgressEvent, WS_METHODS } from "@sparky/contracts";
...
describe("environment RPC", () => {
  ...
  it.effect("fails an in-flight unary request when its session closes", () =>
    Effect.gen(function* () {
  ...
  it.effect("waits for a session that is still being established", () =>
    Effect.gen(function* () {
      const client = {
        [WS_METHODS.cloudGetRelayClientStatus]: () =>
          Effect.succeed({ status: "available", version: "2026.6.0" }),
