...
import { useArchivedProjects } from "../archivedProjectsState";
...
export function useProjects(): ReadonlyArray<EnvironmentProject> {
  ...
  const archivedWorkspaceRoots = useMemo(
    () =>
      new Set(
        archivedProjects.map((project) =>
          project.workspaceRoot.trim().replace(/\\/g, "/").replace(/\/+$/, "").toLocaleLowerCase(),
        ),
      ),
    [archivedProjects],
  );
  return useMemo(
    ...
    [archivedWorkspaceRoots, projects],
  );
}
