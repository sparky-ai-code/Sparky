...
describe("MessagesTimeline", () => {
  ...
  it("renders an empty favicon placeholder while web search is loading", () => {
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildProps()}
        timelineEntries={[
          {
            id: "entry-web-search-loading",
            kind: "work",
            createdAt: MESSAGE_CREATED_AT,
            entry: {
              id: "work-web-search-loading",
              createdAt: MESSAGE_CREATED_AT,
              label: "Search the web",
              toolTitle: "Search the web",
              toolName: "web_search",
    ...
    expect(markup).toContain('aria-label="Finding relevant sources"');
    expect(markup).toContain("bg-muted/45");
    expect(markup).not.toContain('viewBox="0 0 24 24"');
  });

  it("renders stacked source favicons and searched-web copy from structured results", () => {
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildProps()}
        timelineEntries={[
          {
            id: "entry-web-search-sources",
            kind: "work",
            createdAt: MESSAGE_CREATED_AT,
            entry: {
              id: "work-web-search-sources",
              createdAt: MESSAGE_CREATED_AT,
              label: "Search the web",
              toolTitle: "Search the web",
              toolName: "web_search",
    ...
    expect(markup).toContain("Found relevant sources");
    expect(markup).toContain("Sites searched: docs.example.com, guide.example.org");
    expect(markup).toContain('src="https://cdn.example.com/docs-icon.png"');
    expect(markup).toContain("-ml-1.5");
    expect(markup).not.toContain("stale");
    expect(markup).not.toContain("Tool call");
  });
  it("renders source favicons when a search result is already structured", () => {
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildProps()}
        timelineEntries={[
          {
            id: "entry-web-search-direct-results",
            kind: "work",
            createdAt: MESSAGE_CREATED_AT,
            entry: {
              id: "work-web-search-direct-results",
              createdAt: MESSAGE_CREATED_AT,
              label: "Search the web",
              toolTitle: "Search the web",
              toolName: "web_search",
              ...
              toolData: {
                provider: "Exa",
                results: [{ title: "Sparky", url: "https://sparky.llc/docs" }],
              },
            },
          },
        ]}
    ...
    expect(markup).toContain("Sites searched: sparky.llc");
    expect(markup).toContain('src="https://sparky.llc/favicon.ico"');
