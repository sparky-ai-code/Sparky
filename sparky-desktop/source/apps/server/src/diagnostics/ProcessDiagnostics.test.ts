...
describe("ProcessDiagnostics", () => {
  ...
  it.effect("queries Windows performance counters once instead of once per process", () =>
    Effect.gen(function* () {
      const commands: Array<{ readonly command: string; readonly args: ReadonlyArray<string> }> =
        [];
      const spawnerLayer = Layer.succeed(
            ...
            mockHandle({
              stdout: JSON.stringify([
                ...
                },
                {
                  ...
                  Name: "sparky.exe",
                  CommandLine: "sparky --json-stream",
                  Status: null,
                  WorkingSetSize: 2048,
      ...
      expect(rows.map((row) => row.pid)).toEqual([process.pid, 4242]);
      expect(commands).toHaveLength(1);
      expect(commands[0]?.command).toBe("powershell.exe");
      const script = commands[0]?.args.at(-1) ?? "";
      expect(script.match(/Win32_PerfFormattedData_PerfProc_Process/gu)).toHaveLength(1);
      expect(script).not.toContain('-Filter "IDProcess');
      expect(script).toContain("$perfById");
    }),
  );
