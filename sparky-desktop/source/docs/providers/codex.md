...
In Sparky Settings, name it something obvious:
...
In Sparky Settings, add another Codex provider:
...
Sparky shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Sparky treats it
