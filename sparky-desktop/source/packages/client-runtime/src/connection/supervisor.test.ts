...
import { RelayClientTracer } from "@sparky/shared/relayTracing";
...
describe("EnvironmentSupervisor", () => {
  ...
  it.effect("reconnects an idle session when a heartbeat probe fails", () =>
    Effect.gen(function* () {
      const harness = yield* makeHarness({
        probe: (attempt) =>
          attempt === 1 ? Effect.fail(transient("The idle session is stale.")) : Effect.void,
      });
      const supervisor = yield* EnvironmentSupervisor.make(TARGET_ENTRY, {
        initiallyDesired: true,
      }).pipe(Effect.provide(harness.dependencies));
      ...
      expect(failed).toMatchObject({
        lastFailure: {
          _tag: "ConnectionTransientError",
          reason: "transport",
          message: "The idle session is stale.",
        },
      });
