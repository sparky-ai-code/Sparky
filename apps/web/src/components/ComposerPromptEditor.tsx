...
function ComposerPluginDecorator(props: { pluginId: PluginId }) {
  const plugin = getPluginById(props.pluginId);
  ...
  return (
    ...
    >
      <PluginLogo pluginId={props.pluginId} className={COMPOSER_INLINE_CHIP_ICON_CLASS_NAME} />
      <span className={COMPOSER_INLINE_CHIP_LABEL_CLASS_NAME}>{plugin.name}</span>
    </span>
  );
}
...
class ComposerPluginNode extends DecoratorNode<React.ReactElement> {
  ...
  static override clone(node: ComposerPluginNode): ComposerPluginNode {
    return new ComposerPluginNode(node.__pluginId, node.__key);
  }

  static override importJSON(serializedNode: SerializedComposerPluginNode): ComposerPluginNode {
    return $createComposerPluginNode(serializedNode.pluginId).updateFromJSON(serializedNode);
  }

  constructor(pluginId: PluginId, key?: NodeKey) {
    super(key);
  ...
  override getTextContent(): string {
    return serializePluginToken(this.__pluginId);
  }
  ...
  override decorate(): React.ReactElement {
    return <ComposerPluginDecorator pluginId={this.__pluginId} />;
  }
}

function $createComposerPluginNode(pluginId: PluginId): ComposerPluginNode {
  return $applyNodeReplacement(new ComposerPluginNode(pluginId));
}
...
function $setComposerEditorPrompt(
  ...
  for (const segment of segments) {
    ...
    if (segment.type === "plugin") {
      paragraph.append($createComposerPluginNode(segment.pluginId));
      continue;
    }
