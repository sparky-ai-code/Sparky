...
// Keep the first delta immediate so the assistant row appears promptly, then
// combine subsequent text before committing it to the orchestration event store.
// This bounds visible latency while avoiding one SQLite transaction per token.
const STREAMING_ASSISTANT_BATCH_CHARS = 64;
...
const make = Effect.gen(function* () {
  ...
  const appendBufferedAssistantText = (
    messageId: MessageId,
    delta: string,
    flushAtChars = MAX_BUFFERED_ASSISTANT_CHARS + 1,
  ) =>
    Cache.getOption(bufferedAssistantTextByMessageId, messageId).pipe(
      Effect.flatMap((existingText) =>
        Effect.gen(function* () {
          ...
          if (nextText.length < flushAtChars && nextText.length <= MAX_BUFFERED_ASSISTANT_CHARS) {
            yield* Cache.set(bufferedAssistantTextByMessageId, messageId, nextText);
            return "";
          }

          // Flush a bounded chunk to keep streaming responsive without paying
          // for one orchestration transaction per provider token.
    ...
    Effect.gen(function* () {
      ...
      if (assistantDelta && assistantDelta.length > 0) {
        ...
        if (assistantDeliveryMode === "buffered") {
          ...
          // Deliver the first delta immediately to establish the live message.
          // Keep a marker in the same cache so later deltas can be batched.
          const hasStartedStreamingMessage = Option.isSome(
            yield* Cache.getOption(bufferedAssistantTextByMessageId, assistantMessageId),
          );
          if (!hasStartedStreamingMessage) {
            yield* Cache.set(bufferedAssistantTextByMessageId, assistantMessageId, "");
            yield* orchestrationEngine.dispatch({
              type: "thread.message.assistant.delta",
            ...
            const batchedChunk = yield* appendBufferedAssistantText(
              assistantMessageId,
              assistantDelta,
              STREAMING_ASSISTANT_BATCH_CHARS,
            );
            if (batchedChunk.length > 0) {
              yield* orchestrationEngine.dispatch({
                type: "thread.message.assistant.delta",
                commandId: yield* providerCommandId(event, "assistant-delta-batch"),
                threadId: thread.id,
                messageId: assistantMessageId,
                delta: batchedChunk,
                ...(turnId ? { turnId } : {}),
              ...
              // Retain the started marker after flushing this batch so the next
              // provider delta is accumulated instead of becoming a new first
              // delta.
              yield* Cache.set(bufferedAssistantTextByMessageId, assistantMessageId, "");
            }
          }
