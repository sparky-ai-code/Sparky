...
it.layer(NodeServices.layer)("providerStatusCache", (it) => {
  ...
  it("does not let a fallback refresh replace the cached context window", () => {
    const cachedContext = {
      ...
      options: [{ id: "250k", label: "250k", isDefault: true }],
      currentValue: "250k",
    };
      ...
      models: [
        {
          ...
          capabilities: createModelCapabilities({ optionDescriptors: [cachedContext] }),
        },
      ],
    });
      ...
      models: [
        {
          ...
          capabilities: createModelCapabilities({ optionDescriptors: [fallbackContext] }),
        },
      ],
    });

    const hydrated = hydrateCachedProvider({ cachedProvider, fallbackProvider });
    assert.equal(hydrated.models[0]?.contextWindowSource, "provider");
    assert.deepStrictEqual(hydrated.models[0]?.capabilities?.optionDescriptors, [cachedContext]);
  });
