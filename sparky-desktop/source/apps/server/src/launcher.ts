...
function resolvePackagedIconPath(): string {
  const resourcesPath = (process as NodeJS.Process & { resourcesPath?: string }).resourcesPath;
  if (process.platform === "win32" && resourcesPath) {
    const externalIconPath = NodePath.join(resourcesPath, "icon.ico");
    if (NodeFileSystemSync.existsSync(externalIconPath)) return externalIconPath;
  }
  return NodePath.join(PACKAGE_ROOT, "sparky.ico");
}
...
export function resolveLauncherLayout(homeDir: string): LauncherLayout {
  ...
  return {
    ...
    iconPath: resolvePackagedIconPath(),
  };
}
