...
type PickFileOrFolderOptions = PickFolderOptions & {
  kind: "file" | "folder";
};
...
type GitHubPluginSyncResult = {
  connected: boolean;
  error: string | null;
};

type ExtendedDesktopBridge = DesktopBridge & {
  pickFileOrFolder: (options: PickFileOrFolderOptions) => Promise<string | null>;
...
contextBridge.exposeInMainWorld("desktopBridge", {
  ...
  getGitHubCliStatus: () => ipcRenderer.invoke(IpcChannels.GITHUB_CLI_STATUS_CHANNEL),
  installGitHubCli: () => ipcRenderer.invoke(IpcChannels.GITHUB_CLI_INSTALL_CHANNEL),
  startGitHubCliAuth: () => ipcRenderer.invoke(IpcChannels.GITHUB_CLI_AUTH_CHANNEL),
  disconnectGitHub: () => ipcRenderer.invoke(IpcChannels.GITHUB_AUTH_DISCONNECT_CHANNEL),
  syncGitHubPluginConnection: (input) =>
    ipcRenderer.invoke(IpcChannels.GITHUB_PLUGIN_SYNC_CHANNEL, input),
  listGitHubRepositories: (input) =>
    ipcRenderer.invoke(IpcChannels.GITHUB_CLI_LIST_REPOSITORIES_CHANNEL, input),
  listGitHubPullRequests: (input) =>
  ...
  pickFileOrFolder: (options) =>
    ipcRenderer.invoke(IpcChannels.PICK_FILE_OR_FOLDER_CHANNEL, options),
  ...
  onClerkCallback: (listener: (url: string) => void) => {
    const wrappedListener = (_event: Electron.IpcRendererEvent, url: unknown) => {
      if (typeof url === "string") listener(url);
    };
    ipcRenderer.on(IpcChannels.CLERK_CALLBACK_CHANNEL, wrappedListener);
    return () => ipcRenderer.removeListener(IpcChannels.CLERK_CALLBACK_CHANNEL, wrappedListener);
  },
  ...
  onUpdateState: (listener) => {
    ...
    return () => ipcRenderer.removeListener(IpcChannels.UPDATE_STATE_CHANNEL, wrappedListener);
  },
  preview: {
    ...
    recording: {
      ...
      save: (tabId, mimeType, data) =>
        ipcRenderer.invoke(IpcChannels.PREVIEW_RECORDING_SAVE_CHANNEL, { tabId, mimeType, data }),
      onFrame: (listener) => {
