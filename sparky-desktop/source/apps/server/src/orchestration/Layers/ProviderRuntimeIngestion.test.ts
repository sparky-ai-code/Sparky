...
describe("ProviderRuntimeIngestion", () => {
  ...
  it("streams assistant deltas by default when a turn starts", async () => {
    ...
    const batchedDelta = "x".repeat(20);
    for (const [index, delta] of [batchedDelta, batchedDelta, batchedDelta].entries()) {
      harness.emit({
        type: "content.delta",
        eventId: asEventId(`evt-message-delta-streaming-mode-batched-${index}`),
        provider: ProviderDriverKind.make("codex"),
        createdAt: now,
        threadId: asThreadId("thread-1"),
        turnId: asTurnId("turn-streaming-mode"),
    ...
    expect(
      (await harness.readModel()).threads
        ...
        ?.messages.find((message) => message.id === "assistant:item-streaming-mode")?.text,
    ).toBe("hello live");

    harness.emit({
      type: "content.delta",
      eventId: asEventId("evt-message-delta-streaming-mode-batched-flush"),
      provider: ProviderDriverKind.make("codex"),
      createdAt: now,
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-streaming-mode"),
    ...
    await waitForThread(harness.readModel, (entry) =>
      entry.messages.some(
        (message: ProviderRuntimeTestMessage) =>
          message.id === "assistant:item-streaming-mode" &&
          message.text === `hello live${"x".repeat(80)}`,
      ),
    );
