...
export const createOxlintRuleHarness = (
  ...
  const run: RuleHarness["run"] = Effect.fnUntraced(function* (source: string) {
    ...
    const oxlintBin = NodeProcess.platform === "win32" ? NodeProcess.execPath : oxlintScript;
    const oxlintArgs = ["--config", configPath, sourcePath];
    if (NodeProcess.platform === "win32") oxlintArgs.unshift(oxlintScript);
    ...
    const output = yield* spawnAndCollectOutput(
      ChildProcess.make(oxlintBin, oxlintArgs, { cwd: repoRoot }),
    );
