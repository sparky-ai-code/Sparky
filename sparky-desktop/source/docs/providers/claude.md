...
In Sparky Settings, your Claude provider can stay like this:
...
An empty `Claude HOME path` means Sparky uses your normal home directory.
...
Then add another Claude provider in Sparky:
...
Sparky only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so Sparky keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in Sparky Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Sparky stores the value as a server secret and does not
...
Sparky does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in Sparky.
