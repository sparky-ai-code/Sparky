...
export interface UiPinnedThreadState {
  pinnedThreadKeys: string[];
}
...
export interface UiState
  extends UiProjectState, UiPinnedThreadState, UiThreadState, UiEndpointState {}
...
export function toggleThreadPinned(state: UiState, threadKey: string): UiState {
  ...
  const isPinned = state.pinnedThreadKeys.includes(threadKey);
  return {
    ...state,
    pinnedThreadKeys: isPinned
      ? state.pinnedThreadKeys.filter((key) => key !== threadKey)
      : [threadKey, ...state.pinnedThreadKeys],
  };
}
