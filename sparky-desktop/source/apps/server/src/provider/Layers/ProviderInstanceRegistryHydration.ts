...
export const deriveProviderInstanceConfigMap = (
  ...
  const configMap: Record<string, ProviderInstanceConfig> = {
    ...settings.providerInstances,
  };
  const legacyProviders = settings.providers as unknown as Record<string, unknown>;
  ...
  for (const driver of BUILT_IN_DRIVERS) {
    ...
    const enabled =
      ...
      typeof (legacyConfig as { readonly enabled?: unknown }).enabled === "boolean"
        ? (legacyConfig as { readonly enabled: boolean }).enabled
        : true;
  ...
  return configMap as ProviderInstanceConfigMap;
};
...
const SettingsWatcherLive = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    const settingsChanges = serverSettings.subscribeChanges
      ? Stream.fromSubscription(yield* serverSettings.subscribeChanges)
