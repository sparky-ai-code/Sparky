...
// A single pair of Windows CIM queries is still slower than `ps` on typical
// machines. Keep it bounded without timing out every resource-monitor sample.
...
> {
  const command = [
    "$perfById = @{};",
    "Get-CimInstance Win32_PerfFormattedData_PerfProc_Process -ErrorAction SilentlyContinue | ForEach-Object { $perfById[[uint32]$_.IDProcess] = $_.PercentProcessorTime };",
    "$processes = Get-CimInstance Win32_Process -ErrorAction Stop | ForEach-Object {",
    "$cpu = $perfById[[uint32]$_.ProcessId];",
