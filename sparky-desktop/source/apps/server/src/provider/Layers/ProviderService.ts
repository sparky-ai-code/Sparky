...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  const prepareMcpSession = (threadId: ThreadId, providerInstanceId: ProviderInstanceId) =>
    Effect.gen(function* () {
      ...
      yield* Effect.sync(() =>
        McpProviderSession.clearMcpProviderSession(threadId, authSessionId ?? undefined),
      );
  ...
  const clearMcpSession = (threadId: ThreadId) =>
    Effect.gen(function* () {
      const authSessionId = yield* currentAuthSessionId;
      yield* McpSessionRegistry.revokeActiveMcpThread(threadId, authSessionId ?? undefined);
      yield* Effect.sync(() =>
        McpProviderSession.clearMcpProviderSession(threadId, authSessionId ?? undefined),
      );
    });
