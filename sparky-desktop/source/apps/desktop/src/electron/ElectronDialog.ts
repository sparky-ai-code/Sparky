...
export class ElectronDialogPickFileOrFolderError extends Schema.TaggedErrorClass<ElectronDialogPickFileOrFolderError>()(
  "ElectronDialogPickFileOrFolderError",
  {
    ownerWindowId: Schema.NullOr(Schema.Number),
    defaultPath: Schema.NullOr(Schema.String),
    cause: Schema.Defect(),
  ...
  override get message(): string {
    const owner = this.ownerWindowId === null ? "the application" : `window ${this.ownerWindowId}`;
    const defaultPath = this.defaultPath === null ? "no default path" : this.defaultPath;
    return `Failed to open the Electron file or folder picker for ${owner} with ${defaultPath}.`;
  }
}
...
export interface ElectronDialogPickFileOrFolderInput extends ElectronDialogPickFolderInput {
  readonly kind: "file" | "folder" | "any";
}
...
export class ElectronDialog extends Context.Service<
  ElectronDialog,
  {
    ...
    readonly pickFileOrFolder: (
      input: ElectronDialogPickFileOrFolderInput,
    ) => Effect.Effect<Option.Option<string>, ElectronDialogPickFileOrFolderError>;
    readonly confirm: (
...
export const make = ElectronDialog.of({
  ...
  pickFileOrFolder: Effect.fn("desktop.electron.dialog.pickFileOrFolder")(function* (input) {
    const ownerWindowId = Option.match(input.owner, {
      onNone: () => null,
      onSome: (owner) => owner.id,
    });
    ...
    const result = yield* Effect.tryPromise({
      try: () =>
        ...
        new ElectronDialogPickFileOrFolderError({
          ownerWindowId,
          defaultPath,
          cause,
        }),
