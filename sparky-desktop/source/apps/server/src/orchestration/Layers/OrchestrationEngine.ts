...
import { OrchestrationCommand } from "@sparky/contracts";
  ...
  const processEnvelope = (envelope: CommandEnvelope): Effect.Effect<void> => {
    ...
    return Effect.exit(
      Effect.gen(function* () {
        ...
        const authSessionId = envelope.context?.authSessionId;
        const eventBases = authSessionId
          ? decidedEventBases.map((event) => ({
              ...event,
              metadata: {
                ...event.metadata,
                authSessionId,
