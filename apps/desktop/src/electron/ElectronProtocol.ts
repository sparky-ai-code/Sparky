...
export function makeDesktopContentSecurityPolicy(input: DesktopProtocolRegistrationInput): string {
  ...
  // the build-configured account, relay, plugin, and OTLP endpoints. Those origins
  // are not known when this response policy is created, so restrict network access
  // by supported secure schemes. HTTPS frames are allowed for the isolated auth
  // broker surface; Clerk code remains cross-origin and never executes in the
  // privileged renderer itself.
  ...
  return [
    ...
    `frame-src 'self' ${input.scheme}: https:`,
    "form-action 'self'",
  ].join("; ");
}
