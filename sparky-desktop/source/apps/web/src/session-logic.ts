...
export function deriveWorkLogEntries(
  ...
  for (const activity of ordered) {
    ...
    if (isPluginAuthorizationToolActivity(activity)) continue;
...
function isPluginAuthorizationToolActivity(activity: OrchestrationThreadActivity): boolean {
  if (
  ...
  const toolName = asTrimmedString(data?.toolName) ?? asTrimmedString(payload?.toolName);
  return toolName?.toLowerCase() === "sparky_request_plugin_authorization";
}

function normalizePluginToolData(data: Record<string, unknown>): unknown {
  const item = asRecord(data.item);
  if (!item) return data.item;
  const input = asRecord(item.arguments) ?? asRecord(item.input);
  const pluginId =
    asTrimmedString(item.pluginId) ??
    asTrimmedString(input?.pluginId) ??
    asTrimmedString(data.pluginId);
  return pluginId ? { ...item, pluginId } : item;
}
...
function toDerivedWorkLogEntry(activity: OrchestrationThreadActivity): DerivedWorkLogEntry {
  ...
  if (itemType === "mcp_tool_call") {
    if (data?.item !== undefined) {
      entry.toolData = normalizePluginToolData(data);
    }
  } else if ((itemType === "dynamic_tool_call" || itemType === "web_search") && data !== null) {
    ...
    // Web-search items retain their structured result so the timeline can
    // render the searched sites without exposing the raw tool call.
    entry.toolData = data;
  }
  if (itemType) {
