...
function getIconPath(
  ...
  if (platform === "darwin") return undefined; // macOS uses .icns from app bundle
  const ext = platform === "win32" ? "ico" : "png";
  return Option.getOrUndefined(iconPaths[ext]);
}
...
function windowIntersectsDisplay(
  windowBounds: DesktopAppSettings.DesktopWindowBounds,
  displayBounds: DisplayBounds,
): boolean {
  return (
...
// A self-contained startup splash, shown while the backend cold-boots. Inlined
// as a data URL so it needs no bundled asset and no backend — pure CSS, no JS.
...
export const make = Effect.gen(function* () {
  ...
  > {
    ...
    const iconPath = getIconPath(iconPaths, environment.platform);
    ...
    const recoveryBounds =
      "x" in initialBounds && "y" in initialBounds
        ...
        : {
            ...initialBounds,
            x: displayBounds[0]?.x ?? 0,
            y: displayBounds[0]?.y ?? 0,
          };
    ...
    // Reapply after native window creation so Windows refreshes the taskbar
    // image from the external ICO instead of retaining a stale pinned shortcut.
    if (environment.platform === "win32" && iconPath !== undefined) {
    ...
    const openExternalUrl = (rawUrl: unknown): void => {
      if (Option.isSome(ElectronShell.parseSafeExternalUrl(rawUrl))) {
        void runPromise(electronShell.openExternal(rawUrl));
      }
    };
    const attachExternalPopupNavigation = (popup: Electron.BrowserWindow): void => {
      let handled = false;
      const handleNavigation = (event: Electron.Event | null, url: string): void => {
    ...
    window.webContents.on("did-create-window", (popup, details) => {
      if (details.url === "about:blank") {
        attachExternalPopupNavigation(popup);
      }
    });
    ...
    bindFirstRevealTrigger(revealSubscribers, () => {
      ...
      if (persistedSettings.mainWindowMaximized) {
        ...
        const isOnConnectedDisplay = displayBounds.some((display) =>
        ...
        if (displayBounds.length > 0 && !isOnConnectedDisplay) {
          // Windows can preserve a maximized window's old monitor location even
          // when the saved normal bounds are valid. Recover visibly on the
          // current desktop instead of leaving only the startup splash on-screen.
          window.unmaximize();
          window.setBounds(recoveryBounds);
          void runPromise(
            logWindowWarning("saved maximized window was off-screen; restored default bounds"),
          );
        }
      }
