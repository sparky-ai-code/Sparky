...
import { replacePluginTokensWithNames } from "./plugins/pluginCatalog";
...
function ChatViewContent(props: ChatViewProps) {
  ...
  useEffect(() => {
    ...
    const threadActivityAt = [
      ...
      if (latest === null || Date.parse(candidate) > Date.parse(latest)) return candidate;
      return latest;
    }, null);
    if (threadActivityAt === null) return;

    const lastVisitedAt = activeThreadLastVisitedAt ? Date.parse(activeThreadLastVisitedAt) : NaN;
    if (!Number.isNaN(lastVisitedAt) && lastVisitedAt >= Date.parse(threadActivityAt)) return;
  ...
  const onSend = async (e?: { preventDefault: () => void }) => {
    ...
    let titleSeed = replacePluginTokensWithNames(trimmed);
    if (!titleSeed) {
