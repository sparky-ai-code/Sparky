...
import { faviconUrlsForOrigin } from "~/lib/favicon";
...
function PluginMentionChip({ pluginId }: { pluginId: PluginId }) {
  const plugin = getPluginById(pluginId);
  if (!plugin) return null;
  return (
    <span
      className="mx-0.5 inline-flex items-center gap-1 rounded-md border border-border/70 bg-muted/70 px-1.5 py-0.5 align-baseline text-xs font-medium text-foreground"
      title={plugin.description}
      data-message-plugin-chip="true"
    >
      <PluginLogo pluginId={plugin.id} className="size-3.5" />
...
const UserMessageBody = memo(function UserMessageBody(props: {
  ...
  const renderPluginAwareMarkdown = (text: string, keyPrefix: string): ReactNode => {
    const tokenPattern = /@plugin:([a-z]+)(?=$|[\s.,!?;:)\]}])/g;
    ...
    while ((match = tokenPattern.exec(text)) !== null) {
      const plugin = getPluginById(match[1] ?? "");
      if (!plugin) continue;
      ...
      nodes.push(
        <PluginMentionChip
          key={`${keyPrefix}:plugin:${plugin.id}:${pluginCount}`}
          pluginId={plugin.id}
        />,
      );
    ...
    if (cursor < text.length) {
      nodes.push(renderInlineMarkdownSegment(text.slice(cursor), `${keyPrefix}:text:${cursor}`));
    }
    return <div className="whitespace-pre-wrap wrap-break-word">{nodes}</div>;
  };
  ...
  if (props.terminalContexts.length > 0) {
    ...
    if (props.text.length > 0) {
      inlineNodes.push(renderPluginAwareMarkdown(props.text, "user-message-terminal-context"));
  ...
  return renderPluginAwareMarkdown(props.text, "user-message");
});
...
function webSearchResultPreviews(data: unknown): WebSearchResultPreview[] {
  ...
  const output =
    typeof result?.output === "string"
      ...
      : typeof record.output === "string"
        ? record.output
        : "";
  const directResults = Array.isArray(record.results)
    ? record.results
  ...
  const parsedResults =
    parsed && typeof parsed === "object" && !Array.isArray(parsed)
      ...
      : null;
  const candidates = Array.isArray(directResults)
    ? directResults
    : Array.isArray(parsedResults)
      ? parsedResults
      : [...output.matchAll(/https?:\/\/[^\s)]+/gu)].map(([url]) => ({ url }));
  const previews: WebSearchResultPreview[] = [];
  ...
  for (const candidate of candidates) {
    ...
    const domain = parsedUrl.hostname.replace(/^www\./iu, "");
    if (!domain || seenDomains.has(domain)) continue;
    seenDomains.add(domain);
    const faviconUrl =
      typeof value.favicon_url === "string" && value.favicon_url.trim()
        ? value.favicon_url.trim()
        : null;
    previews.push({
      title: typeof value.title === "string" ? value.title.trim() : domain,
...
function WebSearchSourceFavicon({
  source,
  index,
}: {
  readonly source: WebSearchResultPreview;
  readonly index: number;
}) {
  const [loaded, setLoaded] = useState(false);
  const [faviconIndex, setFaviconIndex] = useState(0);
  const faviconUrls = [
    ...(source.faviconUrl ? [source.faviconUrl] : []),
    ...faviconUrlsForOrigin(source.url, 32),
  ].filter((url, index, urls) => urls.indexOf(url) === index);
  const faviconUrl = faviconUrls[faviconIndex];
  const failed = faviconUrl === undefined;
  const className = cn(
    "relative flex size-4 shrink-0 items-center justify-center overflow-hidden rounded-full border border-background bg-muted/45",
    index > 0 && "-ml-1.5",
  );
  if (failed) {
    return (
  ...
  return (
    <span className={className} title={source.domain}>
      <img
        ...
        aria-hidden="true"
        className={cn(
          "relative size-full object-contain p-px transition-opacity duration-150",
          !loaded && "opacity-0",
        )}
        src={faviconUrl}
...
function WebSearchSourcePlaceholder({ className }: { readonly className?: string }) {
  return (
    <span
      aria-hidden="true"
      className={cn(
...
function WebSearchSourceLogos({
  sources,
  loading,
}: {
  readonly sources: ReadonlyArray<WebSearchResultPreview>;
  readonly loading: boolean;
}) {
  if (sources.length === 0) {
    if (!loading) return null;
    return (
      <span className="flex shrink-0 items-center pl-0.5" aria-label="Finding relevant sources">
        <WebSearchSourcePlaceholder />
      </span>
    );
  }
  return (
    <span
      ...
      aria-label={`Sites searched: ${sources.map((source) => source.domain).join(", ")}`}
    >
      {sources.map((source, index) => (
        <WebSearchSourceFavicon key={source.url} source={source} index={index} />
      ))}
    </span>
  );
}
...
function buildToolCallExpandedBody(
  ...
  if (workEntry.itemType === "web_search" && workEntry.toolData !== undefined) {
    const results = webSearchResultPreviews(workEntry.toolData);
    if (results.length > 0) {
      blocks.push(
        results
          .map((result, index) =>
            [`${index + 1}. ${result.title}`, result.domain, result.snippet, result.url]
              .filter(Boolean)
              .join("\n"),
          )
          .join("\n\n"),
...
function resolvePluginToolId(toolName: string | undefined): PluginId | undefined {
  const normalizedName = toolName?.trim().toLowerCase() ?? "";
  return PLUGINS.find((plugin) => normalizedName.startsWith(`${plugin.id}_`))?.id;
}

function pluginToolPresentationTitle(
  workEntry: Pick<TimelineWorkEntry, "toolName" | "toolData">,
): string | undefined {
  const pluginId = resolveWorkEntryPluginId(workEntry);
  if (pluginId === undefined) return undefined;
  const pluginName = getPluginById(pluginId)?.name;
  if (pluginName === undefined) return undefined;
  return `Use ${pluginName} integration`;
}

function resolveWorkEntryPluginId(
  workEntry: Pick<TimelineWorkEntry, "toolName" | "toolData">,
): PluginId | undefined {
  if (
    ...
    const toolData = workEntry.toolData as Record<string, unknown>;
    const pluginIdCandidates = [
      toolData.pluginId,
      (toolData.rawInput as Record<string, unknown> | undefined)?.pluginId,
      (toolData.arguments as Record<string, unknown> | undefined)?.pluginId,
      (toolData.input as Record<string, unknown> | undefined)?.pluginId,
    ];
    for (const pluginId of pluginIdCandidates) {
      if (typeof pluginId === "string" && getPluginById(pluginId)?.id) {
        return getPluginById(pluginId)?.id ?? undefined;
      }
    }
  }
  return resolvePluginToolId(workEntry.toolName);
}
...
function toolWorkEntryHeading(workEntry: TimelineWorkEntry): string {
  const pluginTitle = pluginToolPresentationTitle(workEntry);
...
const SimpleWorkEntryRow = memo(function SimpleWorkEntryRow(props: {
  ...
  const pluginId = resolveWorkEntryPluginId(workEntry);
  const isWebSearch = workEntry.itemType === "web_search";
  const webSearchSources = isWebSearch ? webSearchResultPreviews(workEntry.toolData) : [];
  const webSearchIsLoading =
    isWebSearch &&
    ...
    >
      <div className="flex select-none items-center gap-1.5 transition-[opacity,translate] duration-200">
        <span className={iconWrapperClass}>
          {webSearchSources.length > 0 || webSearchIsLoading ? (
            <WebSearchSourceLogos sources={webSearchSources} loading={webSearchIsLoading} />
          ) : pluginId ? (
            <PluginLogo pluginId={pluginId} className="size-3.5 opacity-80" />
          ) : (
            <WorkEntryIconSvg
              name={entryIconName}
