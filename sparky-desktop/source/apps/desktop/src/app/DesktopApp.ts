...
import { ensureGitHubCliInstalled } from "../ipc/methods/githubCli.ts";
...
const bootstrap = Effect.gen(function* () {
  ...
  if (!(yield* Ref.get(state.quitting))) {
    // Keep a taskbar-visible window present while the backend boots. The real
    // renderer waits for backend readiness, but leaving the process windowless
    // makes a slow or failed backend look like a background-only app in
    // Windows Task Manager and gives the user no way to bring Sparky forward.
...
const startup = Effect.gen(function* () {
  ...
  yield* Effect.sync(() => {
    void ensureGitHubCliInstalled().catch(() => undefined);
  });
  yield* appIdentity.configure;
