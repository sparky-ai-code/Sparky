...
describe("detectComposerTrigger", () => {
  ...
  it("does not reopen the @ menu from an internal plugin token", () => {
    expect(detectComposerTrigger("@plugin:gmail dawg", "@plugin:gmail dawg".length)).toBeNull();
  });

  it("does not reopen the @ menu when text is typed directly after an internal plugin token", () => {
    expect(detectComposerTrigger("@plugin:gmaildawg ", "@plugin:gmaildawg ".length)).toBeNull();
  });
