...
const pluginQueryTrailingPunctuation = (query: string): string =>
  query.match(/[.,!?;:)\]}]+$/u)?.[0] ?? "";
            ...
            render={
              <Button
                ...
                className={cn(
                  ...
                  props.interactionMode === "plan"
                    ? "bg-foreground/10 text-foreground hover:bg-foreground/15 hover:text-foreground"
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const showWorkspaceFiles = workspaceContextEnabled && environmentId !== null && gitCwd !== null;
  const workspaceEntries = useComposerPathSearch({
    environmentId: showWorkspaceFiles ? environmentId : null,
    cwd: isPathTrigger && showWorkspaceFiles ? gitCwd : null,
    query: isPathTrigger && showWorkspaceFiles ? pathTriggerQuery : null,
  });
  ...
  const composerFooterActionLayoutKey = useMemo(() => {
    ...
    if (showPlanFollowUpPrompt) return prompt.trim().length > 0 ? "plan:refine" : "plan:implement";
  ...
  const isComposerMenuLoading =
    composerTriggerKind === "path" && showWorkspaceFiles && workspaceEntries.isPending;
  ...
  useEffect(() => {
    ...
    lastSyncedPendingInputRef.current = { requestId: nextRequestId, questionId: nextQuestionId };
  ...
  useLayoutEffect(() => {
    ...
    const measureFooterCompactness = () => {
      ...
      return { primaryActionsCompact, footerCompact };
    };
      ...
      try {
        ...
        for (const image of composerImages) {
          ...
          try {
            ...
            if (existingPersisted) stagedAttachmentById.set(image.id, existingPersisted);
          }
        }
        ...
        for (const attachment of fallbackPersistedAttachments) {
          if (currentImageIds.has(attachment.id)) fallbackPersistedIds.push(attachment.id);
        }
  ...
  const resolveActiveComposerTrigger = useCallback(() => {
    ...
    const detectedTrigger = detectComposerTrigger(snapshot.value, snapshot.expandedCursor);
    return {
      snapshot,
      trigger: detectedTrigger ?? composerTrigger,
    };
  }, [composerTrigger, readComposerSnapshot]);

  const openComposerPathPicker = useCallback(() => {
    const snapshot = readComposerSnapshot();
    setComposerHighlightedItemId(null);
    setComposerHighlightedSearchKey(null);
    ...
    setComposerTrigger({
      kind: "path",
      query: "",
      rangeStart: snapshot.expandedCursor,
      rangeEnd: snapshot.expandedCursor,
    });
  ...
  const onSelectComposerPlugin = useCallback(
    (plugin: PluginDefinition) => {
      ...
      const { snapshot, trigger } = resolveActiveComposerTrigger();
      if (!trigger) return;
      const replacement = `${serializePluginToken(plugin.id)}${pluginQueryTrailingPunctuation(trigger.query)} `;
      const replacementRangeEnd = extendReplacementRangeForTrailingSpace(
        snapshot.value,
        trigger.rangeEnd,
        replacement,
  ...
  const addComposerImages = (files: File[]) => {
    ...
    if (nextImages.length === 1 && nextImages[0]) addComposerImage(nextImages[0]);
    else if (nextImages.length > 1) addComposerImagesToDraft(nextImages);
    setThreadError(activeThreadId, error);
  };

  const removeComposerImage = (imageId: string) => removeComposerImageFromDraft(imageId);
  ...
  const scheduleComposerCollapseCheck = useCallback(() => {
    if (!isMobileViewport || mobileComposerExpandInFlightRef.current) return;
    if (composerBlurFrameRef.current !== null) {
      window.cancelAnimationFrame(composerBlurFrameRef.current);
    }
    composerBlurFrameRef.current = window.requestAnimationFrame(() => {
      composerBlurFrameRef.current = null;
      if (mobileComposerExpandInFlightRef.current) return;
      const composerSurface = composerSurfaceRef.current;
      const activeElement = document.activeElement;
      if (activeElement instanceof Element && isInsideComposerFloatingLayer(activeElement)) return;
      if (
        composerSurface &&
  ...
  useImperativeHandle(
    composerRef,
    () => ({
      focusAtEnd: () => composerEditorRef.current?.focusAtEnd(),
      focusAt: (cursor: number) => composerEditorRef.current?.focusAt(cursor),
      insertTextAtEnd: insertComposerTextAtEnd,
      openModelPicker: () => setIsComposerModelPickerOpen(true),
      toggleModelPicker: () => setIsComposerModelPickerOpen((open) => !open),
      ...
      addTerminalContext: (selection: TerminalContextSelection) => {
        ...
        window.requestAnimationFrame(() => composerEditorRef.current?.focusAt(nextCollapsedCursor));
      },
      getSendContext: () => ({
    ...
    >
      ...
      >
        <div
          ...
          className={cn(
            "chat-composer-glass rounded-[20px] border border-border transition-[background-color] duration-200 outline-none focus-within:outline-none focus-within:ring-0",
            ...
            {composerMenuOpen && !isComposerApprovalState && (
              <div className="absolute inset-x-0 bottom-full z-20 mb-2">
                <ComposerCommandMenu
                  ...
                  onPluginSelect={onSelectComposerPlugin}
                />
              </div>
            )}
            ...
            >
              <div className="-m-1 flex min-w-0 flex-1 items-center gap-1 overflow-x-auto p-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                <ComposerAddMenu
                  ...
                  disabled={isConnecting || isComposerApprovalState || projectSelectionRequired}
                  onOpenPicker={openComposerPathPicker}
                  onPlanMode={() => {
                    if (interactionMode !== "plan") {
                      void handleInteractionModeChange("plan");
                    }
                  }}
                />
              ...
              >
                ...
                <ComposerFooterPrimaryActions
                  ...
                  activeThreadProviderDisplayName={null}
                  pendingAction={pendingPrimaryAction}
