...
const testPlatform = NodeProcess.platform;
const testArchitecture = NodeProcess.arch;
...
const hostRuntimeLayer = (env: Record<string, string> = {}) =>
  Layer.mergeAll(
    ...
    Layer.succeed(HostProcessArchitecture, testArchitecture),
...
describe("RelayClient", () => {
  it.effect("resolves explicit overrides before managed and PATH executables", () =>
    Effect.gen(function* () {
      ...
      const overridePath = NodePath.join(baseDir, `override-${testExecutableName}`);
  ...
  it.effect("observes PATH changes after the manager has been constructed", () => {
    ...
    return Effect.gen(function* () {
      ...
      const executablePath = NodePath.join(binDir, testExecutableName);
      const manager = yield* makeCloudflaredRelayClient({
        baseDir,
      });
