...
const COMPOSER_PATH_SEARCH_LIMIT = 24;
...
export function useComposerPathSearch(target: ComposerPathSearchTarget) {
  ...
  const hasQuery = normalizedTarget.query.length > 0;
  ...
  const browseResult = useEnvironmentQuery(
    hasWorkspaceTarget &&
      !hasQuery &&
  ...
  const entries: ProjectEntry[] = hasQuery
    ? [...(searchResult.data?.entries ?? [])]
    : (browseResult.data?.entries ?? []).map((entry) => ({
        path: entry.name,
        // Older servers only returned directories from this browse RPC and did
        // not include a kind field. Treat those legacy results as directories.
  ...
  return {
    ...
    isPending:
      hasQuery && normalizedTarget.query !== debouncedTarget.query
        ? true
