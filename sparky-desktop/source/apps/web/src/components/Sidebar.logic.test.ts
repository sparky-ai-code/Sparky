...
describe("formatThreadRunDuration", () => {
  it("formats a completed run from its turn timestamps", () => {
    expect(
      formatThreadRunDuration(
        {
          latestTurn: makeLatestTurn({
            startedAt: "2026-03-09T10:00:00.000Z",
            completedAt: "2026-03-09T10:01:12.000Z",
          }),
          session: null,
        },
        Date.parse("2026-03-09T10:02:00.000Z"),
      ),
    ).toBe("1m 12s");
  });

  it("keeps counting an active run against the supplied clock", () => {
    expect(
      formatThreadRunDuration(
        {
        ...
        Date.parse("2026-03-09T10:00:58.000Z"),
      ),
    ).toBe("58s");
  });
});
...
describe("hasUnseenCompletion", () => {
  ...
  it("shows completion until the thread receives its first visit marker", () => {
    expect(
      hasUnseenCompletion({
        hasActionableProposedPlan: false,
        hasPendingApprovals: false,
...
describe("resolveThreadStatusPill", () => {
  ...
  it("hides the failed marker after the thread is visited", () => {
    const thread = {
      ...baseThread,
    ...
    expect(hasUnseenError(thread)).toBe(false);
  ...
  it("hides completed after the visit marker reaches the completion", () => {
    expect(
      resolveThreadStatusPill({
        thread: {
          ...baseThread,
  ...
  it("shows completed when a turn finishes without a prior visit marker", () => {
    expect(
      resolveThreadStatusPill({
        thread: {
          ...baseThread,
...
describe("resolveThreadRowClassName", () => {
  it("uses the darker selected palette when a thread is both selected and active", () => {
    ...
    expect(className).toContain("!bg-white/[0.1]");
    expect(className).toContain("hover:!bg-white/[0.14]");
    expect(className).toContain("dark:!bg-white/[0.1]");
    expect(className).not.toContain("bg-accent/85");
  });

  it("uses selected hover colors for selected threads", () => {
    const className = resolveThreadRowClassName({ isActive: false, isSelected: true });
    expect(className).toContain("!bg-white/[0.07]");
    expect(className).toContain("hover:!bg-white/[0.11]");
    expect(className).toContain("dark:!bg-white/[0.07]");
