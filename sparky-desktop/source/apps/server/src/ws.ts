...
type StreamingAssistantMessageEvent = ThreadMessageSentEvent & {
  readonly payload: ThreadMessageSentEvent["payload"] & {
    readonly role: "assistant";
    readonly streaming: true;
  };
};

function isStreamingAssistantMessageEvent(
  event: OrchestrationEvent | undefined,
): event is StreamingAssistantMessageEvent {
  return (
    event?.type === "thread.message-sent" &&
...
/**
 * Collapse only adjacent assistant text deltas for the same projected
 * message. Lifecycle/tool events remain boundaries, and the latest sequence
 * is retained so the client's cursor still advances over every persisted
 * event represented by the merged item.
 */
export function coalesceThreadDetailEvents(
  events: ReadonlyArray<OrchestrationEvent>,
): ReadonlyArray<OrchestrationEvent> {
  const output: OrchestrationEvent[] = [];
  for (const event of events) {
    const previous = output.at(-1);
    if (
      isStreamingAssistantMessageEvent(previous) &&
      isStreamingAssistantMessageEvent(event) &&
      ...
      output[output.length - 1] = {
        ...
        payload: {
          ...
          ...(event.payload.attachments === undefined && previous.payload.attachments !== undefined
...
const THREAD_MESSAGE_COALESCE_WINDOW = Duration.millis(16);
const THREAD_MESSAGE_COALESCE_MAX_CHUNK = 128;
const coalesceThreadDetailStream = <E, R>(
  stream: Stream.Stream<OrchestrationEvent, E, R>,
): Stream.Stream<OrchestrationEvent, E, R> =>
  stream.pipe(
    Stream.groupedWithin(THREAD_MESSAGE_COALESCE_MAX_CHUNK, THREAD_MESSAGE_COALESCE_WINDOW),
    Stream.map(coalesceThreadDetailEvents),
...
const makeWsRpcLayer = (
  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const dispatchAsCurrentSession = (command: OrchestrationCommand) =>
        orchestrationEngine.dispatch(command, { authSessionId: currentSessionId });
        ...
        Effect.gen(function* () {
          ...
          const bootstrapProgram = Effect.gen(function* () {
            if (bootstrap?.createThread) {
              yield* dispatchAsCurrentSession({
                type: "thread.create",
            ...
            return yield* dispatchAsCurrentSession(finalTurnStartCommand);
          });
      ...
      const dispatchNormalizedCommand = (
        ...
        const dispatchEffect =
          normalizedCommand.type === "thread.turn.start" && normalizedCommand.bootstrap
            ...
            : dispatchAsCurrentSession(normalizedCommand).pipe(
                Effect.mapError((cause) =>
